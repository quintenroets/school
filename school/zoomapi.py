...
class ZoomApi:
    ...
    def get_content(course_id):
        form_url = (
            f"https://ufora.ugent.be/d2l/lms/remoteplugins/lti/launchLti.d2l?ou={course_id}"
        ...
        if b"errorCode" in post_content:
            ...
            info = {
                k: Parser.between(
                    post_content, f'{k}:"'.encode(), '"'.encode()
                ).decode()
                for k in info_items
            }
            info["X-XSRF-TOKEN"] = Parser.between(
                post_content, b'X-XSRF-TOKEN", value:"', b'"'
            ).decode()
