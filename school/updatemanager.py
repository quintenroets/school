...
class UpdateManager:
    ...
    def process_updates(contentmanager):
        contentmanager.new_topic_sections = [s for s in contentmanager.sections if s.changed and not s.only_subfolders]
        ...
        if constants.update_content:
            c = contentmanager
            Path.content(c.coursemanager.course.name, c.coursemanager.part).write(c.content_bytes)
