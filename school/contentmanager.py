    ...
    def get_updates(self, content, content_old):
        ...
        while queue:
            ...
            for key, subskey in [("ModuleId", "Modules"), ("TopicId", "Topics")]:
                ...
                queue += self.extract_new_items(
                    subcontent, subcontent_old, key, titles, content
                )
    ...
    def extract_new_items(self, items, items_old, key, titles, content=None):
        ...
        for i, item in enumerate(items):
            ...
            if item != item_old:
                ...
                difs = [
                    k
                    for k in ["LastModifiedDate", "SortOrder", key]
                    if item.get(k) != item_old.get(k)
                ]
...
class Section:
    def __init__(self, titles, items, content, coursemanager):
        ...
        self.time = (
            content
            and content.get("LastModifiedDate")
            and timeparser.parse(content["LastModifiedDate"], "")
        )
        self.changed = any([i.changed for i in self.items])

        if content and (html := content.get("Description", {}).get("Html")):
            html_item = Item(
                (content, {}, titles, len(items) + 1), len(items) + 1, self
            )
